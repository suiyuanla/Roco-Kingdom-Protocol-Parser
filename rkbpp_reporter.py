...
class BattleConsoleReporter:
    def __init__(self, *, logger: SessionLogger) -> None:
        ...
        self.start_announced: bool = False
    ...
    def handle(self, row_index: int, row: dict[str, Any], parsed_info: dict[str, Any]) -> None:
        ...
        _HANDLERS = {
            ...
            "action_resolve":     lambda ri, so: self._emit_action_resolve(ri, so.get("detail") or {}),
            "special_refresh":    self._on_special_refresh,
            "server_action_ack":  self._on_action_ack,
            "turn_control":       lambda ri, so: self._emit(ri, f"回合控制包: phase_code={(so.get('detail') or {}).get('phase_code')}"),
            "inner200_commit":    self._on_inner200,
            "inner51_event":      self._on_inner51,
        }
        handler = _HANDLERS.get(kind)
        if handler: handler(row_index, summary_obj) if kind not in ("state_update",) else handler(row_index, record, summary_obj)

    # ------------------------------------------------------------------
    # 事件处理
    # ------------------------------------------------------------------

    def _on_inner390(self, ri: int, obj: dict[str, Any]) -> None:
    ...
    def _on_state_update(self, ri: int, record: dict[str, Any], obj: dict[str, Any]) -> None:
        wrappers = proto.dedupe_state_wrappers(obj.get("wrappers") or [])
    ...
    def _on_skill_select(self, ri: int, obj: dict[str, Any]) -> None:
        d = obj.get("detail") or {}
    ...
    def _on_skill_declare(self, ri: int, obj: dict[str, Any]) -> None:
        d = obj.get("detail") or {}
        if d.get("skill_id") is not None or d.get("action_name"):
            self._emit(ri, f"服务器广播动作: {self._fmt_action_or_skill(d)}")

    def _on_special_refresh(self, ri: int, obj: dict[str, Any]) -> None:
        d = obj.get("detail") or {}
        parts = ["面板刷新"]
        if d.get("action_name"): parts.append(f"动作={d.get('action_name')}")
        ...
        if opts: parts.append(f"技能列表={opts}")
        self._emit(ri, " | ".join(parts))

    def _on_action_ack(self, ri: int, obj: dict[str, Any]) -> None:
        d = obj.get("detail") or {}
        if not (d.get("skill_id") is not None or d.get("action_name") or d.get("state_wrappers")): return
        parts = [f"动作确认: {self._fmt_action_or_skill(d)}"]
        if d.get("current_hp") is not None:  parts.append(f"当前HP={d.get('current_hp')}")
        if d.get("energy_after") is not None: parts.append(f"当前能量={d.get('energy_after')}")
        ws = d.get("state_wrappers") or []
        if ws: parts += [f"实体={ws[0].get('name')}", f"技能={self._fmt_dynamic_skills(ws[0])}"]
        self._emit(ri, " | ".join(parts))

    def _on_inner200(self, ri: int, obj: dict[str, Any]) -> None:
    ...
    def _on_inner51(self, ri: int, obj: dict[str, Any]) -> None:
        d = obj.get("detail") or {}
    ...
    def _maybe_emit_battle_start(self, ri: int) -> None:
        if self.start_announced or self.opening_1316 is None or self.opening_131a is None: return
        ff, ef = self._match_opening_active(self.opening_131a)
        if ff is None or ef is None: return
    ...
    def _emit_snapshot(self, ri: int, wrappers: list[dict[str, Any]], *, opening: bool = False) -> None:
        wrappers = proto.dedupe_state_wrappers(wrappers)
        ...
        for side, w in (("我方", ff), ("敌方", ef)):
            spd = (w.get("battle_stats") or [None]*6)[:6][-1]
    ...
    def _emit_action_resolve(self, ri: int, detail: dict[str, Any]) -> None:
        ...
        if not primary and not damage and not energy: return
        ...
        parts  = [f"{actor}行动"]
        if primary.get("skill_id") is not None: parts.append(f"技能={self._fmt_skill(primary)}")
        ...
        if damage.get("target_hp_after") is not None: parts.append(f"目标剩余HP={damage.get('target_hp_after')}")
        if damage.get("overflow") not in {None, 0}: parts.append(f"溢出={abs(int(damage['overflow']))}")
        if detail.get("effect_ids"): parts.append("状态ID=" + "/".join(str(x) for x in detail["effect_ids"]))
        if detail.get("has_defeat"): parts.append("包含击杀/退场事件")
        self._emit(ri, " | ".join(parts))
    ...
    def _fmt_skill(self, d: dict[str, Any]) -> str:
        sid = d.get("skill_id"); name = d.get("skill_name") or "未知技能"
    ...
    def _fmt_dynamic_skills(self, w: dict[str, Any]) -> str:
        ...
        for it in w.get("dynamic_skills") or []:
            slot = it.get("slot")
            if slot is None or not (1 <= int(slot) <= 4): continue
            sid = it.get("skill_id"); name = proto.skill_name(sid) or str(sid)
    ...
    def _slot_key(self, w: dict[str, Any]) -> int:
        slot = w.get("slot"); return int(slot) if slot is not None else -1
    ...
    def _match_opening_active(self, wrappers: list[dict[str, Any]]) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
        ...
        if ff is None and wrappers: ff = next((it for it in wrappers if int(it.get("slot") or 0) != 0), wrappers[0])
        if ef is None and wrappers: ef = next((it for it in wrappers if it is not ff), None)
        return ff, ef